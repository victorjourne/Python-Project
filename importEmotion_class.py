...
# test ViewEmotion with several image
several_images = Data(df[df['emotion']==1].sample(10))
...
one_image = Data(df.iloc[4:5])
one_image.zoom(2)
one_image.ViewEmotion()    
C1 = convout1(one_image.data_image)
C1 = np.squeeze(C1)
print("C1 shape : ", C1.shape)
nrow = int(np.sqrt(nb_filters+.25)-0.5)+1
for i in range(nb_filters):
    ax = fig.add_subplot(nrow,nrow+1,i+1)
    pixels = np.squeeze(convout1(one_image.data_image))
    ax.imshow(pixels.reshape((24,24)), cmap='gray')
    ax.grid(color='r', linestyle='-', linewidth=1)
    ax.axis('off')
    i = i+1
